...
export class DashboardService {
  ...
  async getAdminSummary() {
    ...
    const byStatus = {
      [FieldStatus.ACTIVE]: fields.filter(
        (f) => f.status === FieldStatus.ACTIVE
      ).length,
      [FieldStatus.AT_RISK]: fields.filter(
        (f) => f.status === FieldStatus.AT_RISK
      ).length,
      [FieldStatus.COMPLETED]: fields.filter(
        (f) => f.status === FieldStatus.COMPLETED
      ).length,
    };
    const byStage = {
